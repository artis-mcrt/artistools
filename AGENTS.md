...
- Do not add a class unless it makes the code much simpler or much more robust. Three fields is the minimum for a `NamedTuple` or a dataclass. For two values, use a plain tuple or separate variables. A type alias is welcome at any size: `type TimeRange = tuple[float, float]` names the tuple one time and keeps the signatures that use it in step.
