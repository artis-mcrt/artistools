...
- Read a compressed ARTIS output file with `at.zopen` or `at.zopenpl`. Do not write your own code for the `.gz`, `.xz`, and `.zst` formats. Two exceptions apply. A reader that is faster is permitted: give the measurements in the commit message. A reader that must not search for a compressed sibling can call `at.misc.fileio.polars_source`, which holds the format rule that `zopenpl` also uses. For an example, see `scan_lines`, which reads the path that `read_wsv` has already resolved.
