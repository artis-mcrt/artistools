...
- Do not leave commented-out code. When you replace or disable something, delete it — git has the old version. Ruff's `commented-out-code` rule is deliberately both ignored and unfixable, so nothing catches this for you. Deleting dead or commented-out blocks near what you are editing is in scope and welcome, since the package has no public API.
