...
- Do not add a class unless it makes the code much simpler or much more robust. A `NamedTuple` or a dataclass that holds two or three values is not wanted. Use a plain tuple or separate variables. A type alias is different and is welcome: `type TimeRange = tuple[float, float]` gives the tuple a name and keeps the signatures that use it in step.
