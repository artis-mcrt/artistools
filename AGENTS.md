...
- Read a compressed ARTIS output file with `at.zopen` or `at.zopenpl`. Do not write your own code for the `.gz`, `.xz`, and `.zst` formats. A reader that is faster is permitted: give the measurements in the commit message. A reader that must not search for a compressed sibling calls `polars_source`, which holds the format rule that `zopenpl` also uses.
