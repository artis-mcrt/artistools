...
- In a package module, import each name from the module that defines it, e.g. `from artistools.inputmodel.inputmodel_misc import get_modeldata`. The `__init__.py` of a package imports its plot modules, thus an import from the package can close an import cycle. The packages `artistools.misc` and `artistools.atomic` import no other part of artistools at module level, thus `from artistools.misc import get_nu_grid` is permitted.
- A package whose core module has the same name is an exception, e.g. `artistools.lightcurve.lightcurve`. From a different package, write `from artistools.lightcurve import get_band_lightcurve` and not `from artistools.lightcurve.lightcurve import get_band_lightcurve`. The `__init__.py` of such a package re-exports the core names first and imports its plot modules after an `# isort: split` line, thus a name exists before a cycle comes back. A module of the package itself imports from the core module, because its own package is not complete at that moment.
...
- If two modules need a name from each other, import the name in the function that uses it. For an example, see `artistools/lightcurve/viewingangleanalysis.py`.
