...
- Do not add a class unless it makes the code much simpler or much more robust. A `NamedTuple` or a dataclass that holds two or three values is not wanted. Use a plain tuple, e.g. `tuple[float, float]`, or separate variables. A function that returns several values returns a tuple.
