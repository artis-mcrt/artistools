    ...
    with contextlib.suppress(FileNotFoundError):
        ...
        for nts, mgi in sorted(estimators.keys()):
            ...
            for strnuc, a in zip(arr_strnuc, arr_a):
                abund = estimators[(nts, mgi)][f"populations_{strnuc}"]
            ...
            abund = estimators[(nts, mgi)].get(f"populations_{strnuc}", 0.0)
            if "Ye" in estimators[(nts, mgi)]:
                ...
                for popkey, abund in estimators[(nts, mgi)].items():
                    if (
                        popkey.startswith("populations_")
                        and "_" not in popkey.removeprefix("populations_")
                        and abund > 0.0
                    ):
