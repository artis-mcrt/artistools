...
def get_completions_script() -> str:
    """Return a shell tab-completion script for every artistools command."""
    import subprocess

    proc = subprocess.run(["register-python-argcomplete", "__MY_COMMAND__"], capture_output=True, text=True, check=True)
    ...
    commands = [*DISPATCHERSCRIPTS, *sorted(get_script_subcommands())]

    # the #compdef line of the function block names the commands that the script completes.
    # Thus the whole output takes the names in place of the placeholder
    lines = [
        "#!/usr/bin/env zsh",
        "# automatically generated by artistools completions",
        strfunctiondefs.replace("__MY_COMMAND__", " ".join(commands)) + strsplit,
        *(strcommandregister.replace("__MY_COMMAND__", command) for command in commands),
    ]
...
def setup_completions(*args: t.Any, **kwargs: t.Any) -> None:  # ruff:ignore[unused-function-argument]
    """Write a shell tab-completion script to the standard output.

    The package folder is read-only on many installations, thus the command writes no file. The
    instructions go to the standard error, thus a redirect of the standard output gives a clean script.
    """
    print(get_completions_script())

    print(
        "\nTo enable completions, write this script to a file. Then source that file from your shell:", file=sys.stderr
    )
    print("\n.zshrc:", file=sys.stderr)
    print("artistools completions > ~/.artistoolscompletions.sh", file=sys.stderr)
    print('source "$HOME/.artistoolscompletions.sh"', file=sys.stderr)
    print("autoload -Uz compinit && compinit", file=sys.stderr)

    print("\n.bashrc:", file=sys.stderr)
    print("artistools completions > ~/.artistoolscompletions.sh", file=sys.stderr)
    print("source ~/.artistoolscompletions.sh", file=sys.stderr)
