...
def make_plot(args: argparse.Namespace) -> None:
    ...
    df = at.inputmodel.get_modeldata(modelpath=Path(args.modelpath), derived_cols=["volume", "rho"])[0].collect()
    ...
    plt.plot(df_plot[xcol], df_plot[f"{args.yaxis}_{args.xaxis}"])

    plt.xlabel("mass number" if xcol == "A" else "charge number")
    ...
    pdf_name = f"model_{args.yaxis}vs{args.xaxis}.pdf"
...
def addargs(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("-modelpath", type=Path, default=Path())
