...
def timeshift_double_decay(
    ...
    elfracsum_before = sum(
        a["specfrac"][:, indexofatomicnumber[zparent - i]] for i in range(3)
    )
    isofracsum_before = sum(
        a["isofrac"][:, indexofisotope[(zparent - i, numnucleons)]]
        for i in range(3)
    )
...
def main():
    ...
    with open(f"{model}/model.txt", "w") as f:
        f.write(str(a["nd"]) + "\n")
        f.write(str(a["time"]) + "\n")
