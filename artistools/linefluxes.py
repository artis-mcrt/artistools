...
def get_line_fluxes_from_pops(
    ...
    for feature in emfeatures:
        ...
        dfnltepops = at.nltepops.read_files(
            modelpath, filterexpr=pl.col("Z") == feature.atomic_number & pl.col("ion_stage") == feature.ion_stage
                ...
                for modelgridindex in modeldata.index:
                    try:
                        levelpop = dfnltepops.filter(
                            pl.col("modelgridindex")
                            == modelgridindex & pl.col("timestep")
                            == timestep & pl.col("Z")
                            == feature.atomic_number & pl.col("ion_stage")
                            == feature.ion_stage & pl.col("level")
                            == upperlevelindex
                        )["n_NLTE"].item(0)
